...
public class Menu : MonoBehaviour {
    ...
    void Start () {
        ...
        Debug.Log("NOMBRE D'OPTIONS:" + options.Length);
        ...
        for (int i = 0; i < options.Length; i++)
        {
            ...
            //GameObject obj = (GameObject)Instantiate(option);

            GameObject obj = optionsObj[i];
            if (!horizontal)
            {
                obj.transform.position = new Vector2(pos.x , pos.y - i * sep);
                //obj.GetComponent<Text>().text = options[i];
            }
            if (horizontal)
            {
    ...
    public void Disappear(bool stay)
    {
        ...
        {
            if(t.transform.parent==gameObject.transform)
            {
